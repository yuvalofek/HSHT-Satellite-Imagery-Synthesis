...
from dataset.set_n import get_loader, get_train_test
print(f'Loading {args.ic} image collection')
paths, tr_idx, test_idx = get_train_test(args.data_root, random_state=214, ic=args.ic, shuffle=False)
...
def evaluate(args):
    ...
    #psnr_list = []
    with torch.no_grad():
        for i, (images, gt_image, inj ) in enumerate(tqdm(all_loader)):
            ...
            for idx, image in enumerate(inj):
                img = image.cpu().numpy()
                with rasterio.open(f'./output/set{i:03}_inj{idx}.tif','w', driver='GTiff', height=img.shape[1],
                              width=img.shape[2], count=img.shape[0], dtype=img.dtype) as dst:
                    dst.write(img)
