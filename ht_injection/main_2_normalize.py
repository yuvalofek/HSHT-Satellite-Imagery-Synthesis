...
paths, tr_idx, test_idx = get_train_test(args.data_root, random_state=214, ic=args.ic)
...
def train(args, epoch):
    ...
    t = time.time()
    for i, (images, gt_image, inj) in enumerate(train_loader):
...
def test(args, epoch):
    ...
    with torch.no_grad():
        for i, (images, gt_image, inj) in enumerate(test_loader):
            ...
            out = model(images, inj) ## images is a list of neighboring frames
            out = torch.cat(out)
            gt = torch.cat(gt)
