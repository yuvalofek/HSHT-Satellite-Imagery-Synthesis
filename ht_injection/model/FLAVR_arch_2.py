...
class UNet_3D_3D(nn.Module):
    def __init__(self, block , n_inputs, n_outputs, batchnorm=False , joinType="concat" , upmode="transpose", channels=3):
        ...
        unet_3D = importlib.import_module(".resnet_3D_2" , "model")
        ...
        self.decoder = nn.Sequential(
            Conv_3d(nf[0]+1, nf[1] , kernel_size=3, padding=1, bias=True, batchnorm=batchnorm),
        ...
        self.lr_injection = nn.MaxPool2d((8, 8))

    def forward(self, images, injection):

        images = torch.stack(images , dim=2)
        injection = torch.stack(injection)
        ...
        # downscale injection and append as channel to x_4 -> lowest dim result of the encoder
        inj = torch.unsqueeze(self.lr_injection(injection), dim=1)
        x_4 = torch.cat((x_4, inj.repeat(1,1,2,1,1)), dim=1)
