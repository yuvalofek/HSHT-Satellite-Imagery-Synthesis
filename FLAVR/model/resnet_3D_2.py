...
class BasicStem(nn.Sequential):
    ...
    def __init__(self):
        super().__init__(
            nn.Conv3d(channels, 64, kernel_size=(channels, 7, 7), stride=(1, 2, 2),
                padding=(channels//2, 3, 3), bias=useBias),
            batchnorm(64),
...
class Conv2Plus1D(nn.Sequential):
    ...
    def __init__(self,
        ...
        super(Conv2Plus1D, self).__init__(
            nn.Conv3d(in_planes, midplanes, kernel_size=(channels//2, 3, 3),
                      stride=(1, stride, stride), padding=(0, padding, padding),
                      bias=False),
            # batchnorm(midplanes),
            nn.ReLU(inplace=True),
            nn.Conv3d(midplanes, out_planes, kernel_size=(channels, 1, 1),
